...
layout (input_attachment_index = 0, set = 2, binding = 0) uniform subpassInputMS inputDepth;

layout (set = 3, binding = 0, rgba32ui) uniform coherent uimage2D colorBuffer;
layout (set = 3, binding = 1, rgba16ui) uniform coherent uimage2D depthBuffer;
layout (set = 3, binding = 2, rgba8) uniform coherent image2D visibilityBuffer;
layout (set = 3, binding = 3, r32ui) uniform coherent uimage2D lock;
...
void main() {
	const float sceneDepth = (subpassLoad(inputDepth, 0).r + subpassLoad(inputDepth, 1).r
			+ subpassLoad(inputDepth, 2).r + subpassLoad(inputDepth, 3).r) * 0.25;
